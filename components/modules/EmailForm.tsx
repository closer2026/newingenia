  ...
  return (
    <Card className="ni-surface rounded-sm">
      ...
      <CardContent className="space-y-3">
        ...
        <div><Label>Ton</Label><div className="mt-2 flex gap-2 text-xs"><span className="rounded-sm bg-[#1f1f1f] px-3 py-1 text-white">Professionnel</span><span className="rounded-sm border px-3 py-1">Cordial</span><span className="rounded-sm border px-3 py-1">Formel</span><span className="rounded-sm border px-3 py-1">Bref</span></div></div>
        ...
        <Button onClick={generate} className="w-full rounded-sm bg-[#1f1f1f] hover:bg-black">{loading ? "Generation..." : "Generer l'email"}</Button>
      </CardContent>
    </Card>
  );
}
